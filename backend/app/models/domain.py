...
class ConfessionLog(SQLModel, table=True):
    ...
    # New V3.2 Fields
    future_aspiration: Optional[str] = Field(None, description="Optional future goal to link with past failure")
...
class VoidReceiptLog(SQLModel, table=True):
    ...
    id: uuid.UUID = Field(default_factory=uuid.uuid4, primary_key=True)
    soul_id: Optional[uuid.UUID] = Field(default=None, index=True, description="Caster's soul ID, hidden from drift API")
    ...
    resonance_count: int = Field(default=0, description="Total resonance signals received")
    new_resonance_count: int = Field(default=0, description="Unseen resonance signals since last login")
    created_at: datetime = Field(default_factory=lambda: datetime.now(timezone.utc))
